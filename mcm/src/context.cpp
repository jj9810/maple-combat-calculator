...
void InternalStat::add(const InternalStat& other) {
    str_fixed += other.str_fixed;
    str_percent += other.str_percent;
    dex_fixed += other.dex_fixed;
    dex_percent += other.dex_percent;
    int_fixed += other.int_fixed;
    int_percent += other.int_percent;
    luk_fixed += other.luk_fixed;
    luk_percent += other.luk_percent;
    hp_fixed += other.hp_fixed;
    hp_percent += other.hp_percent;
    mp_fixed += other.mp_fixed;
    mp_percent += other.mp_percent;
    att_fixed += other.att_fixed;
    att_percent += other.att_percent;
    mag_fixed += other.mag_fixed;
    mag_percent += other.mag_percent;
...
maple_combat_calculator::shared::MCCStat SimulationContext::get_current_total_stat() const {
    ...
    // 최종 스탯 = 고정값 * (1 + 백분율 / 100)
    // 공식 API의 기본 응답은 이미 보정된 값이지만, 시뮬레이션 중 추가되는 버프를 처리하기 위함
    result.set_str(std::floor(internal.str_fixed * mcc::to_multiplier(internal.str_percent)));
    result.set_dex(std::floor(internal.dex_fixed * mcc::to_multiplier(internal.dex_percent)));
    result.set_int_(std::floor(internal.int_fixed * mcc::to_multiplier(internal.int_percent)));
    result.set_luk(std::floor(internal.luk_fixed * mcc::to_multiplier(internal.luk_percent)));
